...
public class Singleton {
	...
	// Sol : so we do null check and make s also private so cant access outside class and static so not depend on object
...
/* we cant make s as final becz final variable should be initialize at the time of declration only after that we cant change 
	private static final Singleton s = new Singleton();
	we should not do this also becz we should not create object untill it is required.
*/
	// Issue : so far this implemention is not thread safe
	...
	// Sol :  make creation logic in syncronized block
	private static Singleton s6 = null;
	public synchronized static Singleton getInstance6() {
		if(s6==null) {
	...
	public synchronized static Singleton getInstance7() {
		if(s6==null) {
			synchronized (Singleton.class) {
				if(s6==null) {
					s6 = new Singleton();
